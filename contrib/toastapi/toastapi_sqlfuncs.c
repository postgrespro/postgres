...
set_toaster(PG_FUNCTION_ARGS)
{
   ...
   Oid tsroid = InvalidOid;
	...
	{
		total_entries++;
		if(strcmp(NameStr(((Form_pg_toaster) GETSTRUCT(tsrtup))->tsrname), tsrname))
		{
	...
	if(d != (Datum) 0)
	{
		d = attopts_get_toaster_opts(relid, attname, "toastrelid");
		if(d == (Datum) 0)
		{
			/* Call tsr->init */
			TsrRoutine *tsr;
			Oid trelid = InvalidOid;
			d = attopts_get_toaster_opts(relid, attname, "toasthandler");
			...
			d = attopts_set_toaster_opts(relid, attname, "toastrelid", str);
			table_close(rel, RowExclusiveLock);
	...
	Assert(len!=0);
	d = attopts_set_toaster_opts(relid, attname, "toasterid", str);
	len = pg_ltoa(tsrhandler, str);
	d = attopts_set_toaster_opts(relid, attname, "toasthandler", str);
