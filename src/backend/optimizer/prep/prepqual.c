...
process_duplicate_ors(List *orlist)
{
	...
	/*
	 * If no winners, we can't do OR-to-ANY transformation to winners.
	 */
	if (winners == NIL)
	{
		List *norlist;
		AlternativeExpr *altexpr;
		...
		{
			norlist = transform_or_to_any(orlist);

			if (norlist == orlist)
				return make_orclause(orlist);

			altexpr = makeNode(AlternativeExpr);
			altexpr->variants = list_make1(make_orclause(orlist));
			altexpr->variants = lappend(altexpr->variants,
								(list_length(norlist) == 1) ?
											(Expr *) linitial(norlist) :
											make_orclause(norlist));
