...
{
	CheckAndCreateToastTable(relOid, reloptions, AccessExclusiveLock,
							 false, NULL);
}

static void
CheckAndCreateToastTable(Oid relOid, Datum reloptions,
						 LOCKMODE lockmode, bool check,
						 Relation old_heap)
{
	...
	else
	{
		...
		for (int i = 0; i < tupDesc->natts; i++)
		{
			...
			tsr->init(rel, attr->atttoaster, InvalidOid, InvalidOid,
					  reloptions, lockmode, check, InvalidOid);
						   ...
						   Oid toasterid, Oid toastrelid)
{
	...
	reltoasterids = PointerGetDatum(construct_array_builtin(new_toasterids, ntoasters + 1, OIDOID));
	reltoastrelids = PointerGetDatum(construct_array_builtin(new_toastrelids, ntoasters + 1, OIDOID));
 ...
 */
static Oid
create_toast_table_generic(Relation rel, Oid toasterid,
