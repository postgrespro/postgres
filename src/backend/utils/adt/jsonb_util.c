...
static void jsonbzDecompressTo(CompressedJsonb *cjb, Size offset);
static bool JsonValueContainsToasted(const JsonValue *jv);
static JsonbValue *getKeyJsonValueFromContainer(JsonContainer *jsc,
...
jsonbMakeToastPointer(JsonbToastedContainerPointerData *ptr)
{
	struct varlena *toast_ptr;// = ptr->toast_ptr;
...
//		return toast_ptr;
	...
	if (ptr->tail_size > 0)
	{
		struct varatt_external_inline toast_ptr_inline;

		toast_ptr_inline.va_external = ptr->ptr;
		toast_ptr_inline.va_inline_size = ptr->tail_size;

		toast_ptr = palloc(TOAST_INLINE_POINTER_SIZE + ptr->tail_size);
		...
		memcpy(VARDATA_EXTERNAL_INLINE(toast_ptr), ptr->tail_data, ptr->tail_size);
	}
	else
	{
		toast_ptr = palloc(TOAST_VERSIONED_POINTER_SIZE);
...
static void
jsonbWriteToastPointer(StringInfo buffer, JsonbToastedContainerPointerData *ptr)
{
	if (ptr->tail_size > 0)
	{
		char		toast_ptr[TOAST_INLINE_POINTER_SIZE];
		struct varatt_external_inline toast_ptr_inline;
	...
	{
		char		toast_ptr[TOAST_VERSIONED_POINTER_SIZE];
...
Datum
jsonbMakeDiffToastPointer(varatt_external_versioned *ptr, int32 diff_offset,
						  int32 diff_len, const void *diff_data)
{
	struct varlena *toast_ptr;
	struct varatt_external_inline toast_ptr_inline;
	struct varatt_external_diff diff;
	int			data_off = offsetof(varatt_external_diff, va_diff_data);

	toast_ptr_inline.va_external = *ptr;
	toast_ptr_inline.va_inline_size = diff_len + data_off;
	diff.va_diff_offset = diff_offset;

	toast_ptr = palloc(TOAST_INLINE_POINTER_SIZE + toast_ptr_inline.va_inline_size);
	SET_VARTAG_EXTERNAL(toast_ptr, VARTAG_ONDISK_INLINE_DIFF);
	memcpy(VARDATA_EXTERNAL(toast_ptr), &toast_ptr_inline, sizeof(toast_ptr_inline));
	memcpy(VARDATA_EXTERNAL_INLINE(toast_ptr), &diff, data_off);
	memcpy(VARDATA_EXTERNAL_INLINE(toast_ptr) + data_off, diff_data, diff_len);

	return PointerGetDatum(toast_ptr);
}
					 ...
					 JsonContainerOps *ops, const JsonValue *binary)
{
	...
	if (ops == &jsonbContainerOps && jc->ops == &jsonbzContainerOps)
	{
		...
		if (JsonContainerIsToasted(jc, &jbcptr))
			return jsonbMakeToastPointer(&jbcptr);

		if (JsonContainerIsCompressed(jc, &jbccomp))
		{
...
static JsonbValue *
jsonbFindKeyInObject(JsonContainer *jsc, const char *key, int len, JsonFieldPtr *ptr)
{
	return getKeyJsonValueFromContainer(jsc, key, len, NULL, ptr);
}
			   ...
			   const JsonbValue *jbval, bool unpackBinary)
{
	...
	{
		JsonbValue *obj = pushJsonbValue(pstate, WJB_BEGIN_OBJECT, NULL);

		obj->val.object.uniquified = jbval->val.object.uniquified;
		obj->val.object.valuesUniquified = jbval->val.object.valuesUniquified;
		obj->val.object.fieldSeparator[0] = jbval->val.object.fieldSeparator[0];
		obj->val.object.fieldSeparator[1] = jbval->val.object.fieldSeparator[1];
		obj->val.object.braceSeparator = jbval->val.object.braceSeparator;
		obj->val.object.colonSeparator = jbval->val.object.colonSeparator;
	...
	if (jbval && (seq == WJB_ELEM || seq == WJB_VALUE) && jbval->type == jbvArray)
	{
		...
		arr = pushJsonbValue(pstate, WJB_BEGIN_ARRAY, NULL);

		arr->val.array.uniquified = jbval->val.array.uniquified;
		arr->val.array.elemsUniquified = jbval->val.array.elemsUniquified;
		arr->val.array.elementSeparator[0] = jbval->val.array.elementSeparator[0];
		arr->val.array.elementSeparator[1] = jbval->val.array.elementSeparator[1];
		arr->val.array.elementSeparator[2] = jbval->val.array.elementSeparator[2];
				  ...
				  struct CompressedJsonb *cjb)
{
	...
	if (cjb)
		jsonbzDecompressTo(cjb, cjb->offset + offsetof(JsonbContainer, children));
	...
	if (it->dataProper && cjb)
		jsonbzDecompressTo(cjb, cjb->offset + (it->dataProper - (char *) container));
...
static void
jsonbInitToastedContainerPointer(JsonbToastedContainerPointerData *jbcptr,
								 //JsonContainer *jc, 
...
bool
JsonContainerIsToasted(JsonContainer *jc, 
					   JsonbToastedContainerPointerData *jbcptr)
{
	...
	if (jc->ops == &jsonbzContainerOps)
	{
		...
		{
			if (jbcptr)
				jsonbInitToastedContainerPointer(jbcptr,
		 ...
		 jc->ops == &jsonbzContainerOps) && !JsonContainerIsScalar(jc))
	{
		...
		if (jsonb_toast_fields)
		{
			...
			if (JsonContainerIsToasted(jc, &jbcptr_data) &&
				offsetof(JsonbToastedContainerPointer, data) + TOAST_INLINE_POINTER_SIZE + jbcptr_data.tail_size < (JENTRY_OFFLENMASK >> 1)) /* FIXME */
			{
				...
				padBufferToInt(buffer);
				appendToBuffer(buffer, &jbcptr, offsetof(JsonbToastedContainerPointer, data));
				jsonbWriteToastPointer(buffer, &jbcptr_data);
				...
				offsetof(JsonbToastedContainerPointer, data) + VARSIZE_ANY(jbccomp_data.compressed_data) < (JENTRY_OFFLENMASK >> 1)) /* FIXME */
			{
				...
				padBufferToInt(buffer);
				appendToBuffer(buffer, &jbccomp, offsetof(JsonbToastedContainerPointer, data));
				appendToBuffer(buffer, jbccomp_data.compressed_data, VARSIZE_ANY(jbccomp_data.compressed_data));
...
static void
jsonbzDecompressAll(CompressedJsonb *cjb)
{
#ifndef JSONB_DETOAST_ITERATOR
...
static void
jsonbzDecompressTo(CompressedJsonb *cjb, Size offset)
{
#ifndef JSONB_DETOAST_ITERATOR
	CompressedDatumDecompress(cjb->datum, offset);
#else
...
jsonbzDecompress(JsonContainer *jc)
{
	...
	jsonbzDecompressTo(cjb, cjb->offset + jc->len);
						 ...
						 JsonValue *result, JsonFieldPtr *ptr)
{
	...
	if (JBE_ISCONTAINER(entry) /* && len > JSONBZ_MIN_CONTAINER_LEN */)
	{
		...
		jsonbzDecompressTo(cjb, cjb2.offset + offsetof(JsonbContainer, children));
	...
	{
		jsonbzDecompressTo(cjb, base_offset + offset + length);
		fillJsonbValue(container, index, base_addr, offset, result, ptr);
	}
...
static JsonbValue *
findValueInCompressedJsonbObject(CompressedJsonb *cjb, 
								 ...
								 JsonFieldPtr *ptr)
{
	...
	jsonbzDecompressTo(cjb, base_offset);
	...
	while (stopLow < stopHigh)
	{
		...
		jsonbzDecompressTo(cjb, base_offset + offset + len);
					  ...
					  JsonFieldPtr *ptr)
{
	...
	if (!cjb->datum->compressed)
	{
		...
		return jsonbFindKeyInObject(&jcd, key, len, ptr);
	}
#endif

	jsonbzDecompressTo(cjb, cjb->offset + offsetof(JsonbContainer, children));

	return findValueInCompressedJsonbObject(cjb, key, len, ptr);
}
...
JsonbzArrayIteratorInit(JsonbzArrayIterator *it, CompressedJsonb *cjb)
{
	...
	jsonbzDecompressTo(cjb, cjb->offset + ((char *) &jbc->children - (char *) jbc));
...
#ifndef JSONB_DETOAST_ITERATOR
	cjb->datum = cd;
	len = VARSIZE_ANY_EXHDR(cd->data); // cd->total_len - VARHDRSZ
#else
	cjb->iter = iter;
	len = VARSIZE_ANY_EXHDR(iter->buf->buf);
#endif
...
DatumGetJsonbPC(Datum datum, Json *tmp, bool copy)
{
	...
	iter = create_detoast_iterator((struct varlena *) DatumGetPointer(datum));
...
# ifdef JSONB_FREE_ITERATORS
		if (jsonb_detoast_iterators)
			MemoryContextSwitchTo(oldcxt);
# endif
		return DatumGetJson(PointerGetDatum(datum), &jsonbContainerOps, tmp);
	}
#endif
...
JsonContainer *
jsonbzInitContainerFromDatum(JsonContainer *jc, Datum toasted_val)
{
	JsonContainerData *tjc;
	JsonbContainerHeader header;

	Assert(VARATT_IS_EXTERNAL_ONDISK_ANY(toasted_val) ||
						  ...
						  JsonValue	*object)
{
	...
	while ((tok = JsonIteratorNext(&it, &jsv, true)) != WJB_DONE)
	{
		...
		else if (tok == WJB_VALUE)
		{
			...
			if (JsonValueIsToasted(&jsv, &size))
			{
				size += offsetof(JsonbToastedContainerPointer, data) + TOAST_INLINE_POINTER_SIZE;
				fields[i].value = NULL;
				fields[i].size = size;
				fields[i].status = 't';
				min_values_length += size;
			}
			else
			{
				switch (jsv.type)
					...
					case jbvBinary:
						{
							...
							min_values_length += INTALIGN(offsetof(JsonbToastedContainerPointer, data) + TOAST_VERSIONED_POINTER_SIZE + 3);
							break;
						}
					case jbvString:
	...
	while (total_size > max_size)
	{
		...
		if (max_key_idx < 0 ||
			max_key_size <= offsetof(JsonbToastedContainerPointer, data) + TOAST_VERSIONED_POINTER_SIZE)
		{
			if (pass == 1)
			{
				pass = 2;
		...
		else
		{
				...
				//total_size + INTALIGN(offsetof(JsonbToastedContainerPointer, data) + VARSIZE_ANY(compressed_val) + 3) <= max_size)
			{
				...
				pairs[max_key_idx].value.val.binary.data = jsonbzInitContainerFromDatum(jc, compressed_val);
		...
		Assert(VARSIZE_ANY(toasted_val) == TOAST_VERSIONED_POINTER_SIZE);

		fields[max_key_idx].size = offsetof(JsonbToastedContainerPointer, data) + TOAST_VERSIONED_POINTER_SIZE;
							  ...
							  JsonbIterator *it, uint32 offset, char cmethod)
{
	...
	if (JsonContainerIsToasted(jc, NULL))
	{
		...
		Assert(VARATT_IS_EXTERNAL_ONDISK_VERSIONED(toasted));
		Assert(VARSIZE_ANY(toasted) == TOAST_VERSIONED_POINTER_SIZE);
...
jsonb_toaster_delete_container(Relation rel, JsonContainer *jc)
{
	...
	if (JsonContainerIsToasted(jc, &jbcptr))
	{
		struct varlena *ptr = jsonbMakeToastPointer(&jbcptr);
				...
				if (old_jbv.type == jbvBinary)
				{
					...
					if (JsonContainerIsToasted(newjc, &new_jbcptr))
					{
						if (!JsonContainerIsToasted(oldjc, &old_jbcptr) ||
							memcmp(&new_jbcptr.ptr.va_external, 
								   &old_jbcptr.ptr.va_external, 
								   sizeof(new_jbcptr.ptr.va_external)))
						{
...
jsonb_toaster(Relation rel, Datum new_val, Datum old_val, int max_size, char cmethod)
{
	...
	if (new_js)
	{
		...
		{
			res = jsonb_toaster_save(rel, new_js, max_size, cmethod);
...
bool
JsonbHasExternal(Datum jb)
{
	Json	   *js = DatumGetJsonbPC(jb, NULL, false);
	bool		res =
		JsonContainerIsToasted(JsonRoot(js), NULL) ||
		JsonContainerContainsToasted(JsonRoot(js));
