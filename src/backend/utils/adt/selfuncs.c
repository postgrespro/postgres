...
/*
 * By utilising extended statistics, try to estimate the bucket size of the hash
 * join inner side when the join condition contains two or more clauses.
								 ...
								 Selectivity *innerbucketsize)
{
	...
	while (clauses != NIL)
	{
		...
		/*
		 * Group inner sides of each clause, addressing the same base relation
		 * and try to estimate such a group with extended statistics.
		 * Combine ndistincts, provided by these separate estimations into one.
		 * form new list of join clauses which didn't fit statistical data.
		 */
		foreach(lc, clauses)
		{
			 ...
			 * number of buckets
			 */
			if (bms_is_subset(rinfo->right_relids, inner->relids))
			{
			...
			if (bms_get_singleton_member(relids, &relid))
			{
				...
				Assert(group_relid == relid);
				varinfo = (GroupVarInfo *) palloc(sizeof(GroupVarInfo));
				varinfo->var = expr;
				varinfo->rel = root->simple_rel_array[relid];
				Assert(varinfo->rel != NULL); /* Is it possible ? */
				varinfo->ndistinct = 0.0;
				varinfo->isdefault = false;
				varinfos = lappend(varinfos, varinfo);

				/*
				 * We must return not estimated clauses into the otherclauses.
				 * So, we should remember their source beforehand.
				 */
				saveList = lappend(saveList, rinfo);
			}
			else
				/*
				 * This clause cannot be potentially estimated with multivariate
				 * statistics
				 */
				otherclauses = lappend(otherclauses, rinfo);
		...
		if (list_length(varinfos) < 2)
		{
			 ...
			 * expressions, but we didn't do it right now.
			 * XXX: May ease this restriction right now?
			 */
		...
		for (;;)
		{
			 ...
			 * according to the caller's logic (see final_cost_hashjoin) and
			 * return unused clauses into the further estimation procedure.
			 */
		...
		forboth(lc1, origin_varinfos, lc2, saveList)
		{
			...
			/*
			 * Add not estimated element to the other clauses for further
			 * estimation
			 */
